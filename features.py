...
# Мировые часовые пояса (используется в VK и TG)
WORLD_TIMEZONES = [
...
WORLD_TZ_KEYBOARD = [c0 for c0, _ in WORLD_TIMEZONES[:16]]
...
def _db():
    ...
    try:
        ...
        for row in conn.execute("SELECT id, name, owner, created_at FROM f_teams"):
            db["teams"][row[0]] = {"name": row[1], "owner": row[2], "members": {}, "created_at": row[3]}
        for row in conn.execute("SELECT team_id, uid, role FROM f_team_members"):
            if row[0] in db["teams"]:
                db["teams"][row[0]]["members"][row[1]] = row[2]
        for row in conn.execute("SELECT handle, owner, title, city, schedule, enabled, created_at, last_post FROM f_channels"):
            db["channels"][row[0]] = {
                "owner": row[1], "title": row[2], "city": row[3],
                "schedule": json.loads(row[4]) if row[4] else {},
                "enabled": bool(row[5]), "created_at": row[6], "last_post": row[7],
            }
        for row in conn.execute("SELECT uid, bg_image, text_color, accent_color, style FROM f_card_settings"):
            item = {}
            if row[1] is not None:
                item["bg_image"] = row[1]
            if row[2] is not None:
                item["text_color"] = row[2]
            if row[3] is not None:
                item["accent_color"] = row[3]
            if row[4] is not None:
                item["style"] = row[4]
            db["card_settings"][row[0]] = item
        for row in conn.execute("SELECT uid, name, updated_at, logo, extra FROM f_white_labels"):
            item = {"name": row[1], "updated_at": row[2], "logo": row[3]}
            if row[4]:
                item.update(json.loads(row[4]))
            db["white_labels"][row[0]] = item
        for row in conn.execute("SELECT code, owner, users, rewarded FROM f_referrals"):
            db["referrals"][row[0]] = {
                "owner": row[1],
                "users": json.loads(row[2]) if row[2] else [],
                "rewarded": bool(row[3]),
            }
        for row in conn.execute("SELECT payload FROM f_events ORDER BY id"):
        ...
        for _row in conn.execute("SELECT code, data FROM f_promos"):
            try:
                db["promos"][_row[0]] = json.loads(_row[1])
            except Exception:
                pass
        for _row in conn.execute("SELECT pid, data FROM f_payments"):
            try:
                db["payments"][_row[0]] = json.loads(_row[1])
            except Exception:
                pass
        return db
    finally:
...
def _save_db(db):
    """Сохраняет словарь обратно в SQLite."""
    ...
    try:
        ...
        conn.execute("DELETE FROM f_teams")
        for tid, t in db.get("teams", {}).items():
            conn.execute("INSERT INTO f_teams (id, name, owner, created_at) VALUES (?,?,?,?)",
                         (tid, t.get("name"), t.get("owner"), t.get("created_at")))
            for mid, role in t.get("members", {}).items():
                conn.execute("INSERT INTO f_team_members (team_id, uid, role) VALUES (?,?,?)",
                             (tid, str(mid), role))
        conn.execute("DELETE FROM f_channels")
        for handle, ch in db.get("channels", {}).items():
            conn.execute(
                "INSERT INTO f_channels (handle, owner, title, city, schedule, enabled, created_at, last_post) VALUES (?,?,?,?,?,?,?,?)",
                (handle, ch.get("owner"), ch.get("title"), ch.get("city"),
                 json.dumps(ch.get("schedule", {}), ensure_ascii=False),
                 1 if ch.get("enabled") else 0, ch.get("created_at"), ch.get("last_post")))
        conn.execute("DELETE FROM f_card_settings")
        for uid, cs in db.get("card_settings", {}).items():
            conn.execute("INSERT INTO f_card_settings (uid, bg_image, text_color, accent_color, style) VALUES (?,?,?,?,?)",
                         (str(uid), cs.get("bg_image"), cs.get("text_color"), cs.get("accent_color"), cs.get("style")))
        conn.execute("DELETE FROM f_white_labels")
        for uid, wl in db.get("white_labels", {}).items():
            extra = {k: v for k, v in wl.items() if k not in ("name", "updated_at", "logo")}
            conn.execute("INSERT INTO f_white_labels (uid, name, updated_at, logo, extra) VALUES (?,?,?,?,?)",
                         (str(uid), wl.get("name"), wl.get("updated_at"), wl.get("logo"),
                          json.dumps(extra, ensure_ascii=False) if extra else None))
        conn.execute("DELETE FROM f_referrals")
        for code, r in db.get("referrals", {}).items():
        ...
        if len(events) < existing:
            conn.execute("DELETE FROM f_events")
            new_events = events
        else:
            new_events = events[existing:]
        for ev in new_events:
            conn.execute("INSERT INTO f_events (user_id, event, ts, payload) VALUES (?,?,?,?)",
                         (ev.get("user_id"), ev.get("event"), ev.get("ts"),
                          json.dumps(ev, ensure_ascii=False)))
        conn.execute("DELETE FROM f_settings")
        for key, value in db.get("settings", {}).items():
            conn.execute("INSERT INTO f_settings (key, value) VALUES (?,?)",
...
def _plans_save(d):
    try:
        ...
        logger.warning(f"plans save error: {e}")
...
def get_user_plan(uid):
    ...
    if plan in ("business", "premium") and e.get("expires", 0) < int(_time_mod.time()):
        e["plan"] = "free"
        e["expires"] = 0
        _plans_save(d)
        track(uid, "plan_expired", {"was": plan})
        return "free"
    return plan

def check_trial_expiry(uid):
    return get_user_plan(uid)

def check_ai_limit(uid):
    """(ok, used, limit, plan) — суточный лимит AI-запросов."""
    plan = get_user_plan(uid)
    limit = {"business": AI_LIMIT_BUSINESS, "premium": AI_LIMIT_PREMIUM}.get(plan, AI_LIMIT_FREE)
    d = _plans_load()
    e = d.setdefault(str(uid), {})
    today = _time_mod.strftime("%Y-%m-%d")
    if e.get("ai_day") != today:
        e["ai_day"] = today
        e["ai_used"] = 0
        _plans_save(d)
    used = e.get("ai_used", 0)
    return used < limit, used, limit, plan

def bump_ai_usage(uid):
    d = _plans_load()
    e = d.setdefault(str(uid), {})
    today = _time_mod.strftime("%Y-%m-%d")
    if e.get("ai_day") != today:
...
def set_notification_prefs(uid, **changes):
    ...
    p = db["users"].setdefault(str(uid), {})
    prefs = p.get("notifications")
    # Защита: notifications мог остаться строкой/числом из старой схемы
    if not isinstance(prefs, dict):
        prefs = {
            "enabled": bool(prefs) if prefs is not None else False,
            "time": "08:00", "frequency": "daily", "timezone": "Europe/Moscow",
...
def _api_keys_to_dict(rows):
    """Конвертирует строки SQLite в словарь {digest: item}."""
    keys = {}
    for row in rows:
        keys[row[0]] = {
            "owner": row[1], "name": row[2], "created_at": row[3],
            "created_ts": row[4], "last_used": row[5],
            "salt": row[6], "active": bool(row[7]), "usage_count": row[8] or 0
        }
    return keys

def create_api_key(uid, name="default"):
    """Создает API ключ. Атомарность через транзакции SQLite + антиспам (60 сек)."""
    ...
    try:
        ...
        rows = conn.execute(
            "SELECT digest, owner, name, created_at, created_ts, last_used, salt, active, usage_count "
            "FROM api_keys WHERE owner=? AND created_ts IS NOT NULL",
            (str(uid),)
        ).fetchall()
        for row in rows:
            if now_ts - (row[4] or 0) < 60:
                logger.warning(f"Антиспам: ключ уже создан недавно, пропуск: user={uid}")
                return None, "recent"
        
        # Лимит: максимум 5 активных ключей
        active_count = conn.execute(
            "SELECT COUNT(*) FROM api_keys WHERE owner=? AND active=1", (str(uid),)
        ).fetchone()[0]
        if active_count >= 5:
            logger.warning(f"Превышен лимит API ключей: user={uid}, count={active_count}/5")
            return None, "limit"
        ...
        item = {
            ...
            "created_ts": now_ts, "last_used": None,
            "salt": salt, "active": True, "usage_count": 0
        }
        conn.execute(
            "INSERT OR REPLACE INTO api_keys (digest, owner, name, created_at, created_ts, last_used, salt, active, usage_count) "
            "VALUES (?,?,?,?,?,?,?,?,?)",
            (digest, item["owner"], item["name"], item["created_at"], item["created_ts"],
             item["last_used"], item["salt"], 1 if item["active"] else 0, item["usage_count"])
        )
        conn.commit()
    ...
    try:
        ...
        for row in rows:
            ...
            if row[0] == expected_digest:
                new_last_used = _now()
                new_usage_count = (row[8] or 0) + 1
                conn.execute(
                    "UPDATE api_keys SET last_used=?, usage_count=? WHERE digest=?",
                    (new_last_used, new_usage_count, row[0])
                )
                conn.commit()
                return {
                    "owner": row[1], "name": row[2], "created_at": row[3],
                    "created_ts": row[4], "last_used": new_last_used,
                    "salt": row[6], "active": bool(row[7]), "usage_count": new_usage_count
                }
...
def _load_api_keys_for_owner(uid):
    """Загружает ключи пользователя для dashboard (в формате {digest: item})."""
    conn = get_conn()
    try:
        rows = conn.execute(
            "SELECT digest, owner, name, created_at, created_ts, last_used, salt, active, usage_count "
        ...
        return _api_keys_to_dict(rows)
    finally:
        conn.close()

def _delete_api_key(digest, uid):
    """Удаляет ключ по digest с проверкой владельца."""
    conn = get_conn()
    try:
        cur = conn.execute("DELETE FROM api_keys WHERE digest=? AND owner=?", (digest, str(uid)))
        conn.commit()
...
"""
    ...
    if low.startswith("/channel_remove"):
        if not _business(uid):
        ...
        if len(parts) < 2:
            if not mine:
                _send(uid, _FT(uid, "no_channels"))
            else:
                hint = "Выберите канал для удаления:" if _lang(uid) == "ru" else "Choose a channel to remove:"
                _send(uid, "🗑 " + hint + "\n" + "\n".join(f"/channel_remove {k}" for k in mine.keys()))
            return True
        arg = parts[1].strip()
        cid = arg if arg.startswith("@") else "@" + arg
        if cid not in db["channels"]:
            msg = "Канал не найден. Список: /channels" if _lang(uid) == "ru" else "Channel not found. List: /channels"
            _send(uid, "❌ " + msg)
            return True
        if str(db["channels"][cid].get("owner")) != str(uid):
            msg = "Это не ваш канал." if _lang(uid) == "ru" else "This is not your channel."
            _send(uid, "⛔ " + msg)
            return True
        del db["channels"][cid]
        _save_db(db)
        track(uid, "channel_removed", {"channel_id": cid})
        msg = f"Канал {cid} отключён." if _lang(uid) == "ru" else f"Channel {cid} disconnected."
        _send(uid, "🗑 " + msg)
        return True
...
def render_dashboard(uid=None, error=None):
    ...
    keys = _load_api_keys_for_owner(uid)
    user_keys = [(d, i) for d, i in keys.items()]
    ...
    def api_dashboard():
        ...
        if request.method == "POST":
            ...
            if delete_key and api_key:
                ...
                if item:
                    if _delete_api_key(delete_key, item["owner"]):
