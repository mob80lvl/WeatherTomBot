...
print("🔄 Обновляем features.py...")

with open('features.py', 'r', encoding='utf-8') as f:
    content = f.read()

# Новые языки для features.py (полные словари)
new_langs_features = """,
    "fr": {
...
# Ищем конец блока "zh" в FEATURE_TEXTS
pattern = r'("zh":\s*\{.*?"daily_wind":\s*"💨 强风警告。"\s*\})\s*\}'
match = re.search(pattern, content, re.DOTALL)

if match:
    # Заменяем конец FEATURE_TEXTS, добавляя новые языки
    content = content[:match.end(1)] + "," + new_langs_features + content[match.end(1):]
    
    with open('features.py', 'w', encoding='utf-8') as f:
        f.write(content)
    print("✅ features.py успешно обновлен!")
else:
    print("❌ Маркер не найден в features.py.")
    # Попытаемся найти, что там есть на самом деле
    m = re.search(r'"zh":\s*\{.*?\}', content, re.DOTALL)
    if m:
        print("Найден блок zh, но конец не совпадает с ожидаемым.")
        print("Последние 150 символов блока zh:")
        print(m.group(0)[-150:])
