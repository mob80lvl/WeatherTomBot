...
def index():
    ...
    total_users = conn.execute("SELECT COUNT(DISTINCT uid) FROM f_users").fetchone()[0]
    conn.close()
...
def set_webhook():
    ...
    if webhook_secret:
        url = f"https://api.telegram.org/bot{TELEGRAM_TOKEN}/setWebhook?url={webhook_url}&secret_token={webhook_secret}&allowed_updates=[%22message%22,%22callback_query%22,%22chat_member%22]"
        logger.info("Устанавливаем webhook с секретным токеном")
    else:
        url = f"https://api.telegram.org/bot{TELEGRAM_TOKEN}/setWebhook?url={webhook_url}&allowed_updates=[%22message%22,%22callback_query%22,%22chat_member%22]"
