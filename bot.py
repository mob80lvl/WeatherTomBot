...
from formatting import format_help_text, format_subscription_status, format_trip_forecast_text, format_welcome_status
...
from keyboards import get_city_keyboard, get_language_keyboard, get_main_keyboard, get_notification_keyboard, get_payment_keyboard, get_team_main_keyboard, get_tz_keyboard, get_white_label_keyboard
...
def _tz_list_text(lang):
    from features import WORLD_TIMEZONES as _WTZ
    head = "🌍 Выберите часовой пояс (номер или название):" if lang == "ru" else "🌍 Choose timezone (number or name):"
    return head + "\n" + "\n".join(f"{i}. {lb} ({cz})" for i, (cz, lb) in enumerate(_WTZ, 1))
...
def create_invoice(chat_id, price, b2b_type=None, plan=None, method="stars"):
    lang = get_user_lang(chat_id)
    if b2b_type:
    ...
    if method == "rub":
        rub_amount = price  # уже в копейках
        usd_price = round((rub_amount / 100) / get_usd_rate(), 2)
        title = f"{title} (~ ${usd_price})"
        description = f"{description}\n\n💳 Оплата картой через ЮKassa\n💰 {rub_amount // 100}₽ (≈ ${usd_price})"
    ...
    try:
                    ...
                    if uid:
                        ...
                        if not e.get("channel_welcomed"):
                            ...
                            welcome = (
                                ...
                                "Здесь каждый час:\n"
                                "• посты рубрик о погоде планеты\n"
                                "• видео с описаниями от AI\n"
                                "• факты, наука, юмор, уют\n\n"
                                "🎁 Ваш подарок уже ждёт:\n"
                                "Введите в боте /free_business —\n"
                                "получите 14 дней тарифа Business бесплатно.\n"
                                "А раз вы подписаны на канал — сразу 30 дней. Автоматически ❤️\n\n"
        ...
        if text == '/start':
            if not current_city:
                ...
                _wline = format_welcome_status(chat_id)
                if _wline:
                    send_message(chat_id, _wline, keyboard)
        ...
        if text.startswith("/broadcast"):
            ...
            # Получаем список всех пользователей из SQLite
            all_users = set()
            try:
                import sqlite3
                conn = sqlite3.connect("bot.db")
                all_users.update(row[0] for row in conn.execute("SELECT uid FROM f_users"))
                all_users.update(row[0] for row in conn.execute("SELECT chat_id FROM users"))
                conn.close()
            except Exception as e:
                logger.error(f"BROADCAST: ошибка чтения SQLite: {e}")
            if not all_users:
        ...
        # Ввод часового пояса (номер / название / кнопка)
        if state.get("mode") == "tz":
            from features import WORLD_TIMEZONES as _WTZ
            t = text.strip()
            if t.startswith("/"):
                _clear_user_state(chat_id)
                ...
                return "ok",200
            if t in (T(lang, "notification_back"), "⬅️ Назад", "⬅️ Back"):
                _clear_user_state(chat_id)
                _show_notification_settings(chat_id)
                return "ok",200
            code = None
            labels = {lb: cz for cz, lb in _WTZ}
            if t in labels:
                code = labels[t]
            elif t.isdigit() and 1 <= int(t) <= len(_WTZ):
                code = _WTZ[int(t) - 1][0]
            else:
                low = t.casefold()
                for cz, lb in _WTZ:
                    if low in lb.casefold() or low in cz.casefold():
                        code = cz
                        break
            if code:
                _clear_user_state(chat_id)
                if advanced_features:
                    advanced_features.set_notification_prefs(chat_id, timezone=code)
                send_message(chat_id, f"✅ {code}", get_notification_keyboard(chat_id))
            else:
                send_message(chat_id, _tz_list_text(lang), get_tz_keyboard(chat_id))
            return "ok",200
        ...
        if state.get("mode") == "ai_question":
            ...
            _clear_user_state(chat_id)
            if advanced_features:
                advanced_features.check_trial_expiry(chat_id)
                ok_limit, used, limit, plan = advanced_features.check_ai_limit(chat_id)
                if not ok_limit:
                    msg = (f"🔒 Лимит AI-запросов на сегодня исчерпан ({used}/{limit}).\n\n"
                           f"Лимит обновится завтра в 00:00.\n"
                           f"Хотите больше? Бизнес-подписка даёт 100 запросов в день.")
                    send_message(chat_id, msg, get_main_keyboard(chat_id))
                    return "ok", 200
            if get_current_plan(chat_id) == "free":
                ...
                if answer:
                    advanced_features.bump_ai_usage(chat_id)
        ...
        if text in ("🌍 Часовой пояс", "🌍 Timezone"):
            _set_user_state(chat_id,"tz"); send_message(chat_id,_tz_list_text(lang),get_tz_keyboard(chat_id)); return "ok",200
        if text in ("🌐 Весь список", "🌐 Full list"):
            send_message(chat_id,_tz_list_text(lang),get_tz_keyboard(chat_id)); return "ok",200
