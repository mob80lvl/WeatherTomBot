...
def _handle_state(uid, peer_id, lang, text):
    ...
    try:
        if mode == "city":
            ...
            code = None
            t = text.strip()
            if t.isdigit() and 1 <= int(t) <= len(TZ_LIST):
                code = TZ_LIST[int(t) - 1][0]
            else:
                low = t.casefold()
                for cz, lb in TZ_LIST:
                    if low in lb.casefold() or low in cz.casefold():
                        code = cz
                        break
            if code:
                set_notification_prefs(uid, timezone=code)
                vk_send(peer_id, f"✅ Часовой пояс: {code}", _notif_keyboard(uid, lang))
            else:
                vk_send(peer_id, "❌ Не понял пояс. Напишите номер из списка.", _tz_keyboard(lang))
        elif mode == "fav_add":
...
def _route(uid, peer_id, lang, cmd, text):
    ...
    if cmd == "weather" or low in ("погода", "weather", "current weather"):
        ...
        _set_state(uid, "tz")
        lines = "\n".join(f"{i}. {label}" for i, (code, label) in enumerate(TZ_LIST, 1))
        vk_send(peer_id, f"🌍 Выберите часовой пояс кнопкой\nили напишите номер/название:\n{lines}", _tz_keyboard(lang))
    elif cmd == "fav_add":
